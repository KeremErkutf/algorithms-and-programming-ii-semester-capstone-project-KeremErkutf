...
from utils import generate_graph_layout 
...
def main():
    ...
    st.sidebar.markdown("---")
    st.sidebar.markdown("**Kerem Erkut Çiftlikçi**")
    ...
    if st.button("Run Topological Sort", key="run_algo_button"):
        ...
        # Ensure current_step_index is valid if steps list is very short (e.g., 0 or 1 step)
        if len(st.session_state.sorted_steps) == 0:
            st.session_state.current_step_index = 0 # No steps
        elif st.session_state.current_step_index >= len(st.session_state.sorted_steps):
            st.session_state.current_step_index = len(st.session_state.sorted_steps) - 1 # Clamp to last step if needed

        st.session_state.algo_choice = algo_choice
        
        # Generate initial fixed positions for consistency during steps using the utility function
        st.session_state.initial_pos = generate_graph_layout(st.session_state.graph) 
    ...
    if 'sorted_steps' in st.session_state and len(st.session_state.sorted_steps) > 0:
        ...
        # Only show animation controls if there's more than one step to animate
        if num_steps > 1:
            ...
            st.sidebar.info("Animation controls are available for graphs with multiple steps.")
            st.session_state.is_playing = False # Ensure animation is off

        st.sidebar.markdown("---")
        st.sidebar.subheader("Step Navigation")
        
        # Only show manual navigation buttons and slider if there's more than one step
        if num_steps > 1:
            ...
            st.sidebar.info("Manual step navigation is available for graphs with multiple steps.")
            st.session_state.current_step_index = 0 # Ensure it's valid for index into steps[0]
        ...
        with vis_col:
            ...
            # Ensure current_step_index is within bounds before accessing sorted_steps
            display_step_index = min(st.session_state.current_step_index, num_steps - 1)
            current_step_data = st.session_state.sorted_steps[display_step_index]
        ...
        with info_col:
            ...
            st.markdown(f"**Step:** {display_step_index + 1} / {num_steps}")
        ...
        if st.session_state.is_playing:
            # Animate up to num_steps-1. When it hits num_steps-1, it displays that step,
            ...
            if st.session_state.current_step_index < num_steps - 1: 
                ...
                # Ensure the slider remains on the last step if it ended there
                # st.session_state.current_step_index = num_steps - 1
                st.rerun() 
